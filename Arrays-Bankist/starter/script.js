...
btnTransfer.addEventListener('click', function (e) {
  ...
  const receiverAcc = accounts.find(
    acc => (acc = acc.username === inputTransferTo.value)
  );
...
console.log(arr2); // This one does mutate this orginigal array
...
const checkDogs = function (dogsJulia, dogsKate) {
  ...
  // dogJuliaCorrected.slice(1, -2) or (1, 3)
  const dogs = dogsJuliaCorrected.concat(dogsKate); // use array method concat

  console.log(dogs);

  dogs.forEach(function (dog, i) {
    //function(currentEl, index, array)
    if (dog >= 3) {
...
they want to convert dog ages to human ages and calculate the average age 
...
const calcAverageHumanAge = function (dogAges) {
  ...
  const averageDog = adults.reduce(
...
// Numbers
console.log(movements);
// console.log(movements.sort()); // -> Not Work! it base on Strings
...
labelBalance.addEventListener('click', function () {
  ...
  // Other way to convert querySelector to array
...
// Prefixed ++ operator
let a = 10;
// console.log(a++); // It still return old value, it was 10
...
const { deposits, withdrawals } = accounts
  ...
  .reduce(
    (sums, curr) => {
      ...
      sums[curr > 0 ? 'deposits' : 'withdrawals'] += curr; // other way up there
      return sums;
    },
